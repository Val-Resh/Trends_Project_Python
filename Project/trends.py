...
def analyze_tweet_sentiment(tweet):
    ...
    if words_number == none_count:
        return make_sentiment(None)
    else:
        return make_sentiment(values / average_counter)
...
def find_centroid(polygon):
    ...
    (1.0, 2.0, 0.0)
    """
    def formula_helper(vertice1, vertice2):
        x = latitude(vertice1)
        x1 = latitude(vertice2)
        y = longitude(vertice1)
        y1 = longitude(vertice2)
        return x * y1 - x1 * y
    
    def formula_helper2(vertice1, vertice2, axis):
        if axis == "lat":
            x = latitude(vertice1)
            x1 = latitude(vertice2)
            return x + x1
    ...
    def centroid(polygon, axis):
        if area(polygon) == 0:
            return zero_area(axis)
        else:
            summation = 0
            multiplier = 1 / (6 * area(polygon))
            if axis == "x":
                for i in range(len(polygon) - 1):
                    a = polygon[i]
                    b = polygon[i + 1]
                    k = formula_helper(a, b)
                    z = formula_helper2(a, b, "lat")
                    summation += k * z
            elif axis == "y":
                for i in range(len(polygon) - 1):
                    a = polygon[i]
                    b = polygon[i + 1]
                    k = formula_helper(a, b)
                    z = formula_helper2(a, b, "lon")
                    summation += k * z
            else:
                return "Invalid input."
            return multiplier * summation
        
    return centroid(polygon, "x"), centroid(polygon, "y"), abs(area(polygon))
...
def find_state_center(polygons):
    ...
    -156.21763
    """
    def polygon_var(polygon, value):
        values = tuple(find_centroid(polygon))
        if value == "x":
            return values[0]
        elif value == "y":
            return values[1]
    ...
    def summation(polygons, value):
        total = 0
        for polygon in polygons:
            total += polygon_var(polygon, value)
        return total
    
    area_sum = summation(polygons, "area")
    x_sum = summation(polygons, "x")
    y_sum = summation(polygons, "y")

    centroid_x = x_sum * area_sum / area_sum
    centroid_y = y_sum * area_sum / area_sum
...
def group_tweets_by_state(tweets):
    ...
    for key in us_states:
        states_centroids[key] = find_state_center(us_states[key])
    ...
    def distance_points(point1, point2):
        ...
        x2 = latitude(point2)
        y2 = longitude(point2)
        def sum_square(a1, a2):
            return pow((a2 - a1), 2)
        return math.sqrt(sum_square(x1, x2) + sum_square(y1, y2))

    for tweet in tweets:
        for key in states_centroids:
            point1 = tweet_location(tweet)
            point2 = states_centroids[key]
            smallest_centroids[key] = distance_points(point1, point2)
        lowest_key = min(smallest_centroids, key=smallest_centroids.get)
        
        if lowest_key not in tweets_by_state:
            tweets_by_state[lowest_key] = [tweet]
        else:
            tweets_by_state[lowest_key].append(tweet)
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        ...
        if sentiment_count != 0:
            avg = sentiment_sum / sentiment_count
